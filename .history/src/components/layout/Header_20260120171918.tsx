      ...
      <header className={`fixed top-0 left-0 right-0 z-[100] transition-all duration-500 ${scrolled ? 'py-4 bg-black/80 backdrop-blur-md border-b border-white/5' : 'py-8'}`}>
        <div className="max-w-[1800px] mx-auto px-8 flex justify-between items-center">
          <Link to="/" className="text-white hover:text-white/80 transition-all text-xl md:text-2xl font-light tracking-[0.4em] z-[120] flex items-center gap-1" onClick={closeMenu}>
            <span className="inline-block align-middle" style={{ width: '1.1em', height: '1.1em' }}>
              <LogoO className="w-[1.1em] h-[1.1em] object-contain inline-block align-middle" style={{ minWidth: 27, minHeight: 27, maxWidth: 35, maxHeight: 35, transform: 'scale(1.6)' }} />
            </span>
            <span className="inline-block align-middle ml-2 tracking-[0.4em]">MEDIA</span>
          </Link>
          {/* Hamburger Icon */}
          <div 
            id="hamburger" 
      ...
      >
        <div className="mt-20 flex flex-col gap-6">
          ...
          <div className="flex flex-col gap-4 mb-4 border-b border-white/10 pb-8">
            <Link to="/produkte" onClick={closeMenu} className="text-white/80 text-lg hover:text-[#D4AF37] tracking-widest transition-colors">{t('menu.products')}</Link>
            <Link to="/abo" onClick={closeMenu} className="text-white/80 text-lg hover:text-[#D4AF37] tracking-widest transition-colors">{t('menu.subscriptions')}</Link>
            <Link to="/ueber" onClick={closeMenu} className="text-white/80 text-lg hover:text-[#D4AF37] tracking-widest transition-colors">{t('menu.about')}</Link>
            <Link to="/kontakt" onClick={closeMenu} className="text-white/80 text-lg hover:text-[#D4AF37] tracking-widest transition-colors">{t('menu.contact')}</Link>
          </div>
