

import { useTranslation } from '../TranslationContext';

export default function ProductEnterprise() {
  const { t } = useTranslation();
  return (
    <div className="max-w-3xl mx-auto py-24 px-6 bg-white">
      <h1 className="text-5xl font-light mb-8 text-[#D4AF37] text-center tracking-tight">{t('product.enterprise.detail.title')}</h1>
      <div className="flex justify-center mb-12">
        <div className="h-0.5 w-32 bg-[#D4AF37] rounded-full" />
      </div>
      <div className="space-y-10 text-gray-900 text-xl leading-relaxed font-light">
        <p>{t('product.enterprise.detail.target')}</p>
        <p>{t('product.enterprise.detail.features')}</p>
        <p>{t('product.enterprise.detail.benefits')}</p>
        <p>{t('product.enterprise.detail.tech')}</p>
        <p>{t('product.enterprise.detail.service')}</p>
        <p>{t('product.enterprise.detail.addons')}</p>
      </div>
      <div className="flex justify-center mt-16">
        <a href="/kontakt" className="px-8 py-3 rounded-full bg-[#D4AF37] text-white font-semibold uppercase tracking-widest text-lg shadow hover:bg-black hover:text-[#D4AF37] border border-[#D4AF37] transition">Beratung</a>
      </div>
    </div>
  );
}
